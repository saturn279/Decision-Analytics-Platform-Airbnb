...
class Preprocess:
    ...
    def get_processed_dfs(calendar_df, listings_df):
        listings_df['price'] = listings_df['price'].astype(str).map(lambda x: x.lstrip("$").replace(",", "")).astype(
            float)
        calendar_df['price'] = calendar_df['price'].astype(str).map(lambda x: x.lstrip("$").replace(",", "")).astype(
            float)
        ...
        overview = overview[['available', 'price', 'revenue', 'daily_revenue', 'booking_rate']]
        ...
        columns = ['host_location', 'host_response_time', 'host_response_rate', 'host_acceptance_rate',
                   'host_is_superhost', 'host_neighbourhood', 'host_total_listings_count', 'host_verifications',
                   'host_identity_verified', 'neighbourhood', 'property_type', 'room_type', 'bedrooms',
                   'minimum_nights', 'maximum_nights', 'has_availability', 'availability_30', 'number_of_reviews',
                   'number_of_reviews_ltm', 'number_of_reviews_l30d', 'review_scores_rating', 'review_scores_accuracy',
                   'review_scores_cleanliness', 'review_scores_checkin', 'review_scores_communication',
                   'review_scores_location', 'review_scores_value', 'instant_bookable', 'price', 'price_avg',
                   'price_std', 'beds', 'accommodates', 'reviews_per_month', 'availability_60', 'availability_90',
                   'availability_365', 'revenue', 'booking_rate', 'bathrooms_text', 'price_per_person', 'accom_per_bed',
                   'price_surge_percent', 'price_std_percent']
        ...
        cleaned_df['host_is_superhost'] = cleaned_df['host_is_superhost'].fillna(
            cleaned_df['host_is_superhost'].mode()[0])
...
class Model:
    ...
    def train(self):
        ...
        def get_top_10_host_neighbourhoods(cleaned_df):
            top_10 = [x for x in
                      cleaned_df.host_neighbourhood.value_counts().sort_values(ascending=False).head(10).index]
            for label in top_10:
        ...
        def get_features(cleaned_df):
            ...
            X = dummy_airbnb.drop(['revenue', 'booking_rate', 'price_per_person'], axis=1)
            y = dummy_airbnb[self.target]

            col_sorted = ['price', 'availability_365', 'beds', 'accommodates', 'availability_60',
        ...
        def get_model(cleaned_df):
            ...
            params = {'xgb__max_depth': [2, 3, 4, 5, 6], 'xgb__n_estimators': [50, 100],
                      'xgb__learning_rate': [0.01, 0.3, 0.05, 0.5, 0.7]}
            ...
            self.estimator = gcv_xgb.best_estimator_

        get_model(self.data_interface.get_data()[1])

    def save(self):
        self.data_interface.save_model(self.estimator, self.target)  # return self.estimator

    def get_metrics(self):
        from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
        y_pred = self.estimator.predict(self.X_test)
        self.metrics = {'mean_absolute_error': mean_absolute_error(self.y_test, y_pred),
...
class DataInterface:
    ...
    def get_recommendation_engine(self):
        ...
        if self.storage_type == 'local':
            self.location = self.location if self.location is not None else f'./Data/{self.city.name}'
            return pickle.load(open(f'{self.location}/recommendation.obj', 'rb'))

    def save_recommendation_engine(self, obj):
        import pickle
        if self.storage_type == 'local':
            self.location = self.location if self.location is not None else f'./Data/{self.city.name}'
            return pickle.dump(obj, open(f'{self.location}/recommendation.obj', 'wb'))
...
class ETL:
    ...
    def transform(self):
        self.calender_df, self.listings_df = Preprocess.get_processed_dfs(calendar_df=self.calendar_df,
...
class Listing:
    ...
    def get_processed(self):
        import numpy as np
        return np.array([[self.price, self.availability_365, self.beds, self.accommodates, self.availability_60,
                          self.review_scores_rating, self.reviews_per_month, self.availability_30]]).astype(np.float64)
...
class RecommendationEngine:
    def __init__(self, datainterface, insights):
        ...
        self.revenue_model, self.booking_model = datainterface.get_model(target='revenue'), datainterface.get_model(
            target='booking_rate')
    ...
    def export(self):
        self.datainterface.save_recommendation_engine(self)
...
def create_insights(city: str):
    ...
    text2 = get_text2()
    insight2 = Insight(plot=fig2, title='Effect of verification on booking rate and revenue', text=text2)
    ...
    rects1 = ax.bar(r1, df_grp['booking_rate'], width=barWidth, label='booking_rate', figure=fig5)
    rects2 = ax.bar(r2, df_grp['revenue'], width=barWidth, label='revenue', figure=fig5)
    ...
    # code for insight 7
    fig7 = plt.figure()
    plt.scatter(df['review_scores_rating'], df['revenue'], figure=fig7)
    plt.xlabel('review_scores_rating')
    plt.ylabel('revenue')
    plt.title('review_scores_rating vs revenue')
    insight7 = Insight(plot=fig7, title=' How review ratings affect revenue and booking rate?')
...
if __name__ == '__main__':
    ...
    recommendation = RecommendationEngine(pickle.load(open(f'DataInterface_{city_name}.obj', 'rb')), insights)
